...
@frappe.whitelist()
def create_roster_data():
    #Get all employees, Fetch the list linked documents 
    try:
        all_active_employees = frappe.db.sql(f"""SELECT name from `tabEmployee` where status = 'Active'""",as_dict =1 )
        if all_active_employees:
            yesterday = add_days(today(),-1)
            yesterday = "2023-05-17"
            active_employees = tuple([i.name for i in all_active_employees])
            data_query = f"""    SELECT emp.name as active_employee,la.name as emp_leave,es.name as emp_sch,ar.name as attendance_request,sp.name as shift_perm,sa.name as shift_assign,ts.name as timesheet
                                   FROM `tabEmployee`emp
                                LEFT JOIN `tabLeave Application`la ON emp.name= la.employee
                                    AND '{yesterday}' BETWEEN la.from_date and la.to_date  AND la.docstatus = 1
                                LEFT JOIN `tabEmployee Schedule`es ON emp.name = es.employee
                                    AND es.date = '{yesterday}'
                                LEFT JOIN `tabTimesheet`ts ON emp.name = ts.employee
                                    AND '{yesterday}' BETWEEN ts.start_date and ts.end_date  AND ts.docstatus = 1
                                LEFT JOIN `tabShift Assignment`sa ON emp.name = sa.employee
                                    AND '{yesterday}' BETWEEN sa.start_date and sa.end_date  AND sa.docstatus = 1
                                LEFT JOIN `tabShift Permission`sp ON emp.name = sp.employee
                                    AND sp.date = '{yesterday}' AND sp.docstatus = 1
                                LEFT JOIN `tabAttendance Request`ar ON emp.name = ar.employee
                                    AND '{yesterday}' BETWEEN ar.from_date and ar.to_date AND ar.docstatus = 1
                            WHERE
                                emp.name IN {active_employees}
                            GROUP BY emp.name
                               ORDER BY emp.name	
                           """
            
            data_result = frappe.db.sql(data_query,as_dict = 1)
            print(data_result)
            create_docs(data_result,yesterday)
            return response("Success", 200, {})
        # get the previous date 
    except Exception as e:
        frappe.log_error(frappe.get_traceback(), 'Employee Roster Error')
        

def create_docs(data_pack,yesterday):
    try:
        base_query =  """
            Insert Into
                `tabPassed Assesment`
                (
                     `name`,`employee`, `date`, `leave_application`, `shift_assignment`, `attendance_request`, `employee_schedule`, `shift_permission`, `timesheet`
                )
            Values
        """
        for each in data_pack:
            base_query += f"""
                    (
                        "{each['active_employee']+"_"+yesterday}", "{each['active_employee']}", "{yesterday}", "{each['emp_leave'] or " "}", "{each['shift_assign'] or " "}",
                        "{each['attendance_request'] or " "}", "{each['emp_sch'] or " "}" , "{each['shift_perm'] or " "}",
                        '{each['timesheet'] or " "}'
                    ),"""
        frappe.db.sql(base_query[:-1], as_dict=1)
        frappe.db.commit()
